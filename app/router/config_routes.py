...
async def get_providers(request: Request):
    ...
    auth_token = request.cookies.get("auth_token")
    if not auth_token or not verify_auth_token(auth_token):
        logger.warning("Unauthorized access attempt to providers list")
        raise HTTPException(status_code=403, detail="Not authenticated")
    ...
    try:
        ...
        for name, service in services.items():
            providers.append({
                ...
                "api_keys_count": len(service.config.api_keys),
...
async def get_providers_status(request: Request):
    ...
    auth_token = request.cookies.get("auth_token")
    if not auth_token or not verify_auth_token(auth_token):
        logger.warning("Unauthorized access attempt to providers status")
        raise HTTPException(status_code=403, detail="Not authenticated")
...
async def reload_providers(request: Request):
    ...
    auth_token = request.cookies.get("auth_token")
    if not auth_token or not verify_auth_token(auth_token):
        logger.warning("Unauthorized access attempt to reload providers")
        raise HTTPException(status_code=403, detail="Not authenticated")

    try:
        manager = await get_provider_manager()
        await manager.reload_config()
        return {"success": True, "message": "Providers reloaded successfully"}
    except Exception as e:
        logger.error(f"Error reloading providers: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Error reloading providers: {str(e)}")
