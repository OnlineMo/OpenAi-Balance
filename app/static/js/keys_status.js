...
async function verifyKey(key, button, provider = null) {
  try {
    // 禁用按钮并显示加载状态
    ...
    try {
      // 构建 URL，支持提供商参数
      let url = `/api/keys/verify/${encodeURIComponent(key)}`;
...
async function resetKeyFailCount(key, button, provider = null) {
  try {
    // 禁用按钮并显示加载状态
    button.disabled = true;
    const originalHtml = button.innerHTML;
    ...
    // 构建 URL，支持提供商参数
    let url = `/api/keys/reset-fail-count/${encodeURIComponent(key)}`;
    if (provider && provider !== "all") {
      url += `?provider=${encodeURIComponent(provider)}`;
    }
...
async function executeResetAll(type) {
  ...
  // 构建 URL，支持提供商参数
  const providerParam = currentProvider && currentProvider !== 'all' ? `?provider=${encodeURIComponent(currentProvider)}` : '';
  ...
  for (let i = 0; i < keysToReset.length; i++) {
    ...
    try {
      const data = await fetchAPI(`/api/keys/reset-fail-count/${encodeURIComponent(key)}${providerParam}`, {
        method: "POST",
      });
      if (data.success) {
        successCount++;
    ...
    for (let i = 0; i < keysToVerify.length; i += batchSize) {
      ...
      try {
        const options = {
          ...
          body: JSON.stringify({ keys: batch, provider: currentProvider !== 'all' ? currentProvider : null }),
        };
        const data = await fetchAPI(`/api/keys/verify-batch`, options);
...
async function fetchAndDisplayKeys(type, page = 1) {
    ...
    try {
        ...
        // 添加提供商筛选
        if (currentProvider && currentProvider !== 'all') {
        ...
        const keysInfo = data.keys_info || {};
        if (Object.keys(keys).length > 0) {
            Object.entries(keys).forEach(([key, fail_count]) => {
                // 获取提供商信息
                const keyInfo = keysInfo[key] || {};
                const provider = keyInfo.provider || null;
                const listItem = createKeyListItem(key, fail_count, type, provider);
 ...
 */
function createKeyListItem(key, fail_count, type, provider = null) {
    const li = document.createElement("li");
    ...
    // 提供商标识（仅在"全部"视图中显示）
    const providerBadge = (provider && currentProvider === 'all')
        ? `<span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-blue-50 text-blue-600 border border-blue-200">
            <i class="fas fa-server mr-1"></i>${escapeHtml(provider === 'default' ? '默认' : provider)}
           </span>`
        : '';
    ...
    li.innerHTML = `
        ...
        <div class="flex-grow">
            <div class="flex flex-col justify-between h-full gap-3">
                ...
                <div class="flex flex-wrap items-center gap-2">
                    <button class="flex items-center gap-1 bg-success-600 hover:bg-success-700 text-white px-2.5 py-1 rounded-lg text-xs font-medium transition-all duration-200" onclick="verifyKey('${key}', this, ${provider ? `'${provider}'` : 'null'})"><i class="fas fa-check-circle"></i> 验证</button>
                    <button class="flex items-center gap-1 bg-gray-500 hover:bg-gray-600 text-white px-2.5 py-1 rounded-lg text-xs font-medium transition-all duration-200" onclick="resetKeyFailCount('${key}', this, ${provider ? `'${provider}'` : 'null'})"><i class="fas fa-redo-alt"></i> 重置</button>
...
async function loadProviders() {
  try {
    ...
    if (container && data.providers) {
      ...
      // 添加提供商按钮（排除名为 "default" 的提供商，因为它与内置默认提供商重复）
      Object.entries(data.providers).forEach(([name, providerData]) => {
        // 跳过名为 "default" 的提供商
        if (name.toLowerCase() === 'default') {
          return;
        }
        const total = providerData.valid_keys_count + providerData.invalid_keys_count;
        const btn = document.createElement('button');
        btn.type = 'button';
        btn.className = 'provider-filter-btn px-4 py-2 rounded-lg text-sm font-medium transition-all duration-200 bg-gray-100 text-gray-700 hover:bg-gray-200';
        btn.setAttribute('data-provider', name);
        btn.onclick = () => filterByProvider(name);
        btn.innerHTML = `
          <i class="fas fa-cloud mr-1"></i> ${escapeHtml(name)}
...
/**
 * 按提供商筛选密钥
 * @param {string} provider 提供商名称，'all' 表示全部
 */
function filterByProvider(provider) {
  currentProvider = provider;

  // 更新按钮样式
  const buttons = document.querySelectorAll('.provider-filter-btn');
  buttons.forEach(btn => {
    if (btn.getAttribute('data-provider') === provider) {
      btn.className = 'provider-filter-btn px-4 py-2 rounded-lg text-sm font-medium transition-all duration-200 bg-blue-500 text-white';
    } else {
      btn.className = 'provider-filter-btn px-4 py-2 rounded-lg text-sm font-medium transition-all duration-200 bg-gray-100 text-gray-700 hover:bg-gray-200';
    }
  });
