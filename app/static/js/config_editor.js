...
function initializeSensitiveFields() {
  ...
  // Helper function: Create partial mask (show first 8 and last 4 characters)
  function createPartialMask(value) {
  ...
  function maskField(field) {
    if (field.value && field.value !== MASKED_VALUE && !field.value.includes("...")) {
      field.setAttribute("data-real-value", field.value);
      // Use partial mask for API keys to show distinguishable content
      field.value = createPartialMask(field.value);
    } else if (!field.value) {
  ...
  function unmaskField(field) {
    ...
    // If no data-real-value and value is MASKED_VALUE or partial mask, it might be an initial empty sensitive field, clear it
    else if (
      (field.value === MASKED_VALUE || field.value.includes("...")) &&
...
async function initConfig() {
  try {
    ...
    // 调试日志
    console.log("initConfig: API_KEYS from server =", config.API_KEYS);
    console.log("initConfig: API_KEYS length =", config.API_KEYS ? config.API_KEYS.length : 0);

    // 确保数组字段有默认值
    // 注意：API_KEYS 和 ALLOWED_TOKENS 不设置默认值，保持从后端获取的实际值
    if (!config.API_KEYS || !Array.isArray(config.API_KEYS)) {
      config.API_KEYS = [];
    }

    if (!config.ALLOWED_TOKENS || !Array.isArray(config.ALLOWED_TOKENS)) {
    ...
    if (typeof populateProvidersConfig === "function") {
      populateProvidersConfig(config);
    }
...
function populateForm(config) {
  ...
  if (budgetMapContainer) {
    ...
    console.warn("THINKING_BUDGET_MAP_container not found, skipping budget map initialization");
    // Continue processing other fields instead of returning early
  }
  ...
  if (Array.isArray(config.THINKING_MODELS)) {
    ...
    if (container) {
      ...
      console.warn("THINKING_MODELS_container not found, skipping thinking models initialization");
    }
  }
  ...
  if (Array.isArray(config.API_KEYS)) {
    ...
    currentApiKeyPage = 1;
    console.log("populateForm: allApiKeys count =", allApiKeys.length);
    renderApiKeyPage();
    updateApiKeyPagination();
  } else {
    console.log("populateForm: config.API_KEYS is not an array or is undefined");
    // 确保即使没有 API_KEYS 也更新显示
    allApiKeys = [];
    filteredApiKeys = [];
    updateApiKeyPagination();
  }
...
function handleBulkAddApiKeys() {
  ...
  // 按行分割，过滤空行和空白行，支持任意格式的 API 密钥
  const extractedKeys = bulkText
    .split(/[\r\n]+/)
    .map(line => line.trim())
    .filter(line => line.length > 0);

  if (extractedKeys.length === 0) {
    showNotification("未找到有效的 API 密钥，请每行输入一个密钥", "warning");
    return;
  }

  // 获取目标提供商
  const targetProviderSelect = document.getElementById("apiKeyTargetProvider");
  const targetProvider = targetProviderSelect ? targetProviderSelect.value : "default";

  if (targetProvider === "default") {
    // 添加到默认 API_KEYS 列表
    ...
    // 更新提供商密钥部分
    if (typeof renderProviderKeysSection === "function") {
      renderProviderKeysSection();
    }

    closeModal(apiKeyModal);
    showNotification(`添加/更新了 ${extractedKeys.length} 个密钥到默认列表`, "success");
  } else {
    // 添加到指定提供商
    const providerIndex = providersConfig.findIndex(p => p.name === targetProvider);
    if (providerIndex >= 0) {
      const existingKeys = providersConfig[providerIndex].api_keys || [];
      const combinedKeys = new Set([...existingKeys, ...extractedKeys]);
      providersConfig[providerIndex].api_keys = Array.from(combinedKeys);
      ...
      closeModal(apiKeyModal);
      showNotification(`添加/更新了 ${extractedKeys.length} 个密钥到提供商 "${targetProvider}"`, "success");
    } else {
      showNotification(`未找到提供商 "${targetProvider}"`, "error");
    }
  }
}

/**
 * Updates the provider options in the API key modal
 */
function updateApiKeyTargetProviderOptions() {
  const targetProviderSelect = document.getElementById("apiKeyTargetProvider");
  if (!targetProviderSelect) return;

  // 清除现有选项（保留默认选项）
  while (targetProviderSelect.options.length > 1) {
    targetProviderSelect.remove(1);
  }

  // 添加提供商选项
  if (typeof providersConfig !== "undefined" && Array.isArray(providersConfig)) {
    providersConfig.forEach((provider) => {
      if (provider.enabled) {
        const option = document.createElement("option");
        option.value = provider.name;
        option.textContent = `提供商: ${provider.name}`;
        targetProviderSelect.appendChild(option);
      }
    });
  }
}
...
function collectFormData() {
  ...
  arrayContainers.forEach((container) => {
    ...
    formData[key] = Array.from(arrayInputs)
      ...
      .filter(
        (value) => value && value.trim() !== "" && value !== MASKED_VALUE && !value.includes("...")
      ); // Filter out masked values (both full mask and partial mask)
  });
  ...
  // --- 新增：收集提供商配置 ---
  if (typeof collectProvidersConfig === "function") {
    const providersData = collectProvidersConfig();
    formData["DEFAULT_PROVIDER"] = providersData.DEFAULT_PROVIDER;
    formData["PROVIDERS_CONFIG"] = providersData.PROVIDERS_CONFIG;
  }
  // --- 结束：收集提供商配置 ---
