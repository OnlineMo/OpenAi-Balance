...
class ConfigService:
    ...
    async def get_config() -> Dict[str, Any]:
        ...
        # 调试日志
        logger.info(f"get_config: API_KEYS count = {len(config.get('API_KEYS', []))}")
        logger.info(f"get_config: ALLOWED_TOKENS count = {len(config.get('ALLOWED_TOKENS', []))}")
        return config

    @staticmethod
    async def update_config(config_data: Dict[str, Any]) -> Dict[str, Any]:
        # 关键配置项：如果前端传来空值，但内存中有非空值，则保留内存中的值
        critical_keys = ["API_KEYS", "ALLOWED_TOKENS", "AUTH_TOKEN"]
            ...
            if key in config_data:
                ...
                if isinstance(new_value, list) and len(new_value) == 0:
                    if isinstance(current_value, list) and len(current_value) > 0:
                        logger.warning(
                            f"Skipping update of '{key}': new value is empty but current value is not. "
                            f"Current value will be preserved."
                        )
                        del config_data[key]
    ...
    async def fetch_ui_models() -> List[Dict[str, Any]]:
        ...
        try:
            ...
            models_response = await model_service.get_models(api_key)
